...
class StructureAwareRepresentationLearning(nn.Module):
    ...
    def __init__(self, feature_dims: List[int], shared_dim: int = 512, 
                 ...
                 beta: float = 0.5, use_pretrained: bool = True, device: str = 'cpu'):
        super(StructureAwareRepresentationLearning, self).__init__()
        
        self.modality_encoder = ModalityEncoder(feature_dims, shared_dim, use_pretrained, device)
    ...
    def get_modality_encoder(self):
        """Get modality encoder for external use"""
        return self.modality_encoder
    
    def get_fused_representation(self):
        """Get fused representation module for external use"""
